...
const Mercado = () => {
  ...
  const exportCSV = () => {
    ...
    const header = Object.keys(rows[0] || {id:'',playerId:'',from:'',to:'',amount:'',status:'',date:''}).join(',');
  ...
  const totals = useMemo(() => {
    const sum = (arr: any[]) => arr.reduce((acc, t) => acc + (t.amount ?? t.fee ?? 0), 0);
    return {
      count: filtered.length,
      pending: sum(filtered.filter(t => t.status === 'pending')),
      approved: sum(filtered.filter(t => t.status === 'approved')),
      rejected: sum(filtered.filter(t => t.status === 'rejected')),
  ...
  const filtered = useMemo(() => {
    ...
    if (clubFrom.trim()) list = list.filter(t => (t.fromClubName || t.fromClubId || '').toString().toLowerCase().includes(clubFrom.toLowerCase()));
    if (clubTo.trim()) list = list.filter(t => (t.toClubName || t.toClubId || '').toString().toLowerCase().includes(clubTo.toLowerCase()));
    ...
    return list;
  }, [transfers, status, clubFrom, clubTo, dateFrom, dateTo, minAmount, maxAmount, sortBy]);

  const [pendingCount, setPendingCount] = useState(
    filtered.filter(t => t.status === 'pending').length
  );
  const [status, setStatus] = useState<'all'|'pending'|'approved'|'rejected'>('pending');
  const [clubFrom, setClubFrom] = useState<string>('');
  const [clubTo, setClubTo] = useState<string>('');
  ...
  useEffect(() => {
    const unsub = subscribeGlobal(
      state => state.filtered.filter(t => t.status === 'pending').length,
      setPendingCount
    );
    return () => unsub();
  }, []);

  const filteredTransfers = filtered.filter(transfer => {
        ...
        <div className="bg-gray-800/50 backdrop-blur-sm rounded-xl border border-gray-700/50 p-6 mb-4">
          <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
            ...
            <div>
              ...
              <input className="input mt-1" placeholder="Nombre o ID" value={clubFrom} onChange={e=>setClubFrom(e.target.value)} />
            </div>
            <div>
              <label className="text-sm text-gray-400">Club vendedor</label>
              <input className="input mt-1" placeholder="Nombre o ID" value={clubTo} onChange={e=>setClubTo(e.target.value)} />
            </div>
            <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-4">
            ...
            <div className="flex items-end">
              <button className="btn-secondary w-full" onClick={()=>{ setStatus('all'); setClubFrom(''); setClubTo(''); setDateFrom(''); setDateTo(''); setMinAmount(''); setMaxAmount(''); }}>Limpiar</button>
            </div>
            <div className="flex items-end">
          ...
          <div className="mt-4 text-sm text-gray-400">
            <span className="mr-4">Resultados: <span className="text-white font-medium">{totals.count}</span></span>
            <span className="mr-4">Pendientes: <span className="text-white font-medium">€{totals.pending.toLocaleString()}</span></span>
            <span className="mr-4">Aprobados: <span className="text-white font-medium">€{totals.approved.toLocaleString()}</span></span>
            <span>Rechazados: <span className="text-white font-medium">€{totals.rejected.toLocaleString()}</span></span>
          </div>
        </div>
