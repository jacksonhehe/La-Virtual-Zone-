...
const NewClubModal = ({ onClose, onCreate, dtUsers }: NewClubModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(r as number) || (r as number) < 0 || (r as number) > 100) {
      setError('Reputacion debe estar entre 0 y 100');
      return;
    }
    const fb = typeof fanBase === 'string' ? parseInt(fanBase || '10000', 10) : fanBase;
    if (isNaN(fb as number) || (fb as number) < 0) {
      setError('Base de aficionados invalida');
      return;
    }
      ...
      <div className="relative bg-dark-light rounded-xl shadow-xl w-full max-w-2xl overflow-hidden border border-gray-700 max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800 bg-gray-900">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-xl font-bold text-white">Nuevo club</h3>
              <p className="text-sm text-gray-400">Crea un equipo en la liga</p>
            </div>
          </div>
          <button
            onClick={onClose}
                  ...
                  <div className="space-y-3">
                    <div className="relative">
                      <input
                        ...
                        id="club-logo-upload"
                      />
                      <label htmlFor="club-logo-upload" className="btn-outline w-full flex items-center justify-center cursor-pointer">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      {dtUsers.map(dt => (
                        <option key={dt.id} value={dt.id} disabled={dt.clubId && dt.clubId !== ''}>
                          {dt.username}{dt.clubId ? ' (ocupado)' : ''}
                        </option>
                      ))}
                    </select>
                  </div>
