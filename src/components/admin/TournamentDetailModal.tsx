...
const TournamentDetailModal = ({ tournament, onClose }: TournamentDetailModalProps) => {
  ...
  // Load matches from the independent matches table instead of tournament.matches
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-gray-800 rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="p-6 overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h4 className="font-semibold mb-2">Informacion</h4>
              <div className="text-sm text-gray-300 space-y-1">
                <div>
                  Tipo: <span className="text-gray-400">{tournament.type === 'league' ? 'Liga' : tournament.type === 'cup' ? 'Copa' : 'Amistoso'}</span>
                </div>
                <div>
                  Fechas: <span className="text-gray-400">{formatDate(tournament.startDate)} - {formatDate(tournament.endDate)}</span>
                </div>
                <div>
                  Estado: <span className="text-gray-400">{tournament.status === 'active' ? 'Activo' : tournament.status === 'upcoming' ? 'Proximo' : 'Finalizado'}</span>
                </div>
                <div>
                  Jornadas: <span className="text-gray-400">{tournament.rounds}</span>
                </div>
                {tournament.description && (
                  <div>
                    Descripcion: <span className="text-gray-400">{tournament.description}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="text-sm text-gray-300 grid grid-cols-2 gap-1 max-h-40 overflow-auto p-2 bg-dark rounded">
                {visibleTeams.map((team) => (
                  <div key={team} className="truncate" title={team}>
                    {team}
                  </div>
                ))}
              </div>
          ...
          <div className="mt-6">
            ...
            {!hasMatches ? (
              ...
              <div className="bg-dark rounded border border-gray-700 overflow-auto max-h-[50vh]">
