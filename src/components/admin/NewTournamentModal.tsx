    ...
    try {
      ...
      if (logoFile && !logo) {
        ...
        reader.onload = async readerEvent => {
          ...
          await Promise.resolve(onCreate({ ...payload, logo: finalLogo || undefined }));
          setIsSaving(false);
        };
        reader.readAsDataURL(logoFile);
        return;
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800 bg-gray-900">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400">Crea una nueva competicion</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-900">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <div className="bg-gray-800/60 border border-gray-700/60 rounded-lg p-3 text-sm text-gray-200">
                <div className="text-xs text-gray-400 mb-1">Tipo</div>
                <div className="font-semibold capitalize">{type === 'league' ? 'Liga' : type === 'cup' ? 'Copa' : 'Amistoso'}</div>
              </div>
              <div className="bg-gray-800/60 border border-gray-700/60 rounded-lg p-3 text-sm text-gray-200">
                <div className="text-xs text-gray-400 mb-1">Equipos</div>
                <div className="font-semibold">{teams.length} seleccionados</div>
              </div>
              <div className="bg-gray-800/60 border border-gray-700/60 rounded-lg p-3 text-sm text-gray-200">
                <div className="text-xs text-gray-400 mb-1">Jornadas</div>
                <div className="font-semibold">
                  {type === 'cup' ? 'Auto (copa)' : (rounds || 1)}
                </div>
              </div>
            </div>
                      ...
                      {logoFile && (
                        <div className="mt-2 flex gap-2">
                          <button type="button" onClick={handleLogoUpload} className="btn-primary text-sm px-3 py-1" disabled={isSaving}>
                            {isSaving ? 'Procesando...' : 'Usar esta imagen'}
                          </button>
            ...
            <div className="flex space-x-3 pt-2">
              ...
              <button type="submit" className="flex-1 btn-primary disabled:opacity-60 disabled:cursor-not-allowed" disabled={isSaving}>
                ...
                {isSaving ? 'Guardando...' : 'Crear torneo'}
              </button>
            </div>
          </form>
        </div>
