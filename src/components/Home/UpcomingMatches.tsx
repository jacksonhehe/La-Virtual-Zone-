import { useState, useEffect } from 'react';
import { Link } from 'react-router-dom';
import { ChevronRight } from 'lucide-react';
import { useDataStore } from '../../store/dataStore';
import { formatDate, formatTime } from '../../utils/format';
import { getMatchesByTournament } from '../../utils/matchService';
import { getCupStageLabel, isCupTournament } from '../../utils/matchStages';

const UpcomingMatches = () => {
  const { tournaments, clubs } = useDataStore();
  const [upcomingMatches, setUpcomingMatches] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const loadUpcomingMatches = async () => {
      setLoading(true);
      try {
        const activeTournament = tournaments.find(t => t.status === 'active');

        if (!activeTournament) {
          setUpcomingMatches([]);
          return;
        }

        const matches = await getMatchesByTournament(activeTournament.id);
        const upcoming = matches
          .filter(match => match.status === 'scheduled')
          .sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime())
          .slice(0, 3);

        setUpcomingMatches(upcoming);
      } catch (error) {
        console.error('Error loading upcoming matches:', error);
        setUpcomingMatches([]);
      } finally {
        setLoading(false);
      }
    };

    loadUpcomingMatches();
  }, [tournaments]);

  const activeTournament = tournaments.find(t => t.status === 'active');
  const activeTournamentIsCup = isCupTournament(activeTournament?.type, activeTournament?.name);

  const skeletonCards = Array.from({ length: 3 }).map((_, index) => (
    <div key={index} className="p-4 animate-pulse">
      <div className="h-4 w-32 bg-gray-800 rounded mb-3" />
      <div className="flex items-center justify-between my-3">
        <div className="flex items-center space-x-3">
          <span className="w-8 h-8 bg-gray-800 rounded" />
          <span className="h-4 w-20 bg-gray-800 rounded" />
        </div>
        <span className="text-gray-700">vs</span>
        <div className="flex items-center space-x-3">
          <span className="h-4 w-20 bg-gray-800 rounded" />
          <span className="w-8 h-8 bg-gray-800 rounded" />
        </div>
      </div>
      <div className="h-3 w-40 bg-gray-800 rounded" />
    </div>
  ));

  if (!activeTournament && !loading) {
    return (
      <div className="card p-6">
        <h2 className="text-xl font-bold mb-4">Proximos partidos</h2>
        <p className="text-gray-400">No hay torneos activos en este momento.</p>
      </div>
    );
  }

  if (upcomingMatches.length === 0 && !loading) {
    return (
      <div className="card p-6">
        <h2 className="text-xl font-bold mb-4">Proximos partidos</h2>
        <p className="text-gray-400">No hay partidos programados proximamente.</p>
      </div>
    );
  }

  return (
    <div className="card">
      <div className="p-6 border-b border-gray-800">
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-bold">Proximos partidos</h2>
          <Link
            to="/liga-master/fixture"
            className="text-primary hover:text-primary-light flex items-center text-sm"
          >
            <span>Ver fixture</span>
            <ChevronRight size={16} />
          </Link>
        </div>
      </div>

      <div className="divide-y divide-gray-800">
        {loading ? skeletonCards : upcomingMatches.map((match) => {
          const homeClub = clubs.find(c => c.name === match.homeTeam);
          const awayClub = clubs.find(c => c.name === match.awayTeam);

          return (
            <div key={match.id} className="p-4">
              <div className="text-sm text-gray-300">
                {formatDate(match.date)} - {formatTime(match.date)}
              </div>

              <div className="flex items-center justify-between my-3">
                <div className="flex items-center space-x-3">
                  <img
                    src={homeClub?.logo}
                    alt={`Escudo de ${homeClub?.name || 'club local'}`}
                    className="w-8 h-8 object-contain"
                    loading="lazy"
                  />
                  <span className="font-medium text-white">{homeClub?.name || match.homeTeam}</span>
                </div>
                <span className="text-gray-400 mx-4">vs</span>
                <div className="flex items-center space-x-3">
                  <span className="font-medium text-white">{awayClub?.name || match.awayTeam}</span>
                  <img
                    src={awayClub?.logo}
                    alt={`Escudo de ${awayClub?.name || 'club visitante'}`}
                    className="w-8 h-8 object-contain"
                    loading="lazy"
                  />
                </div>
              </div>

              <div className="text-xs text-gray-400 text-center">
                {activeTournamentIsCup ? getCupStageLabel(match.round) : `Jornada ${match.round}`} | {activeTournament?.name || 'Liga Master'}
              </div>
            </div>
          );
        })}
      </div>
    </div>
  );
};

export default UpcomingMatches;
